...
export default function CalculatorPage() {
  ...
  const handleBuySubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    let totalPayableAmount = Number(
      (shareAmount + brokerCommission + sebonCommission + dpFee).toFixed(2)
    );
    let costPricePerShare = Number(
      (totalPayableAmount / noOfShares).toFixed(2)
    );
    let cost = {
      shareAmount,
      brokerCommission,
